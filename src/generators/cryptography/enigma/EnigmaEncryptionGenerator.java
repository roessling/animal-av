/*
 * EnigmaEncryptionGenerator.java
...
public class EnigmaEncryptionGenerator implements ValidatingGenerator {
	...
	public EnigmaEncryptionGenerator(Locale loc) {
		this.loc = loc;
		this.translator = new Translator("EnigmaEncryptionGenerator", loc);
	}

	public void init() {
		lang = new AnimalScript("Enigma Encryption", "Marius Gassen, Markus Danz", 1024, 768);
		lang.setStepMode(true);
	}

	public String generate(AnimationPropertiesContainer props, Hashtable<String, Object> primitives) {

		ArrayProperties msgProps = (ArrayProperties) props.getPropertiesByName("Message Array");
		...
		Color messageHighlight = (Color) msgProps.get(AnimationPropertiesKeys.CELLHIGHLIGHT_PROPERTY);
		...
		String message = ((String) primitives.get("Nachricht")).toUpperCase(Locale.GERMANY).trim().replace("Ü", "UE")
		...
		// PLAINTEXT
		Text messageTextLabel = lang.newText(new Coordinates(COLUMN_ONE_X, MESSAGE_Y + LABEL_OFFSET),
				translator.translateMessage("inputText"), "messageLabel", null);
		StringArray messageArray = lang.newStringArray(new Coordinates(COLUMN_TWO_X, MESSAGE_Y), message.split(""),
				"messageText", null, ap);
		messageArray.showIndices(false, Timing.INSTANTEOUS, Timing.INSTANTEOUS);

		// CYPHERTEXT
		Text cypherTextLabel = lang.newText(new Coordinates(COLUMN_ONE_X, CYPHER_Y + LABEL_OFFSET),
				translator.translateMessage("outputText"), "cyherTextLabel", null);
		String[] cypher = new String[message.length()];
		Arrays.fill(cypher, " ");
		StringArray cypherArray = lang.newStringArray(new Coordinates(COLUMN_TWO_X, CYPHER_Y), cypher,
				"cypherTextArray", null, ap);
		...
		rmp.set(AnimationPropertiesKeys.FILL_PROPERTY, messageHighlight);
		...
		messageArray.setHighlightFillColor(0, message.length(), messageHighlight, Timing.INSTANTEOUS,
				Timing.INSTANTEOUS);
		cypherArray.setHighlightFillColor(0, message.length(), cypherHighlight, Timing.INSTANTEOUS, Timing.INSTANTEOUS);
		...
		for (int i = 0; i < message.length(); i++) {
			...
			if (i != 0) {
				messageArray.unhighlightCell(i - 1, Timing.INSTANTEOUS, Timing.INSTANTEOUS);
			}
			messageArray.highlightCell(i, Timing.INSTANTEOUS, Timing.INSTANTEOUS);

			char currentCypher = machine.encrypt(c);
			result += currentCypher;
			...
			cypherArray.highlightCell(i, delayCypher, Timing.INSTANTEOUS);
			cypherArray.put(i, Character.toString(currentCypher), delayCypher, Timing.INSTANTEOUS);
		...
		messageArray.unhighlightCell(message.length() - 1, Timing.INSTANTEOUS, Timing.INSTANTEOUS);
		cypherArray.unhighlightCell(message.length() - 1, Timing.INSTANTEOUS, Timing.INSTANTEOUS);
		...
		lang.newText(new Coordinates(120, 140), message, "finalSlide", null);
	...
	public String getName() {
		if (getContentLocale() == Locale.GERMANY)
			return "Enigma Verschlüsselung";
		else
			return "Enigma Encryption";
	}
	...
	public boolean validateInput(AnimationPropertiesContainer props, Hashtable<String, Object> primitives)
		...
		String message = ((String) primitives.get("Nachricht")).toUpperCase();
		String[] rotorA = (String[]) primitives.get("Walze A");
		String[] rotorB = (String[]) primitives.get("Walze B");
		String[] rotorC = (String[]) primitives.get("Walze C");
		String[] reflector = (String[]) primitives.get("Umkehrwalze");
