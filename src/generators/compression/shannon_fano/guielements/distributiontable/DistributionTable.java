...
public class DistributionTable {
	...
	public float[] getProbabilities() {
		...
		float[] probabilities = new float[elements.size()];
		for (int i = 0; i < elements.size(); i++) {
			probabilities[i] = elements.get(i).getProbability();
		}
	...
	/**
	 * Sorts the distribution table according to the frequencies in decending order.
	 */
	public void sort() {
		Comparator<DistributionTableElement> comp = new Comparator<DistributionTableElement>() {

			@Override
			public int compare(DistributionTableElement a,
					DistributionTableElement b) {
