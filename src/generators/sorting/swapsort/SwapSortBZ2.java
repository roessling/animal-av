...
public class SwapSortBZ2 extends AnnotatedAlgorithm implements
  ...
  public String generate(AnimationPropertiesContainer arg0,
    ...
    vars.setGlobal(comp);
    vars.declare("int", recdep, "0");
    vars.setGlobal(recdep);
    ...
    Text compText = lang.newText(new Coordinates(270, 120), "", "complexText",
        null);
  ...
  private void swapsort(int[] array) {
    ...
    // jMarkerProperties);
    // jMarker.hide();
    // count_sum = lg.newText(new Coordinates(40 , 160), "Count = ", "count",
    // null, textProperties);
    // count_sum.hide();
    exec("swapsort");
    lg.nextStep();
  ...
  public String getAnnotatedSrc() {
    return "public int[] swapsort(int[] array){ @label(\"swapsort\")\n"
        + "   int i = 0; @label(\"vars\") @declare(\"int\", \"i\") @set(\"i\", \"0\")\n"
        + "   while(i < array.length - 1) { 	@label(\"while\")  @inc(\""
        + comp
        + "\")"
        + "       int count = countSmallerElements(array , i);	@label(\"CSE\")"
        + "       if (count > 0) {	@label(\"if\") @inc(\""
        + comp
        + "\")"
        + "           array = swap(array , i , count);	@label(\"swap\") @inc(\""
        + assi
        + "\")"
        + "       }	@label(\"ifEnd\")"
        + "       else {	@label(\"else\")"
        + "           i++;	@label(\"elseInc\")@inc(\"i\") @inc(\""
        + assi
        + "\")"
        + "       }	@label(\"elseEnd\")"
        + "   }	@label(\"whileEnd\")"
        ...
        + "}	@label(\"end\")"
        + "private int countSmallerElements(int[] array , int index) {	@label(\"CSEheader\") @openContext"
        + "   int count = 0;	@label(\"CSEvars\") @declare(\"int\", \"count\")"
        + "   for (int i = index + 1; i < array.length; i++) {	@label(\"CSEfor\") @inc(\"i\")  @inc(\""
        + assi
        + "\")"
        + "       if(array[index] > array[i])	@label(\"CSEif\") @inc(\""
        + comp
        + "\")"
        + "           count++;	@label(\"CSEifInc\") @inc(\"count\") @inc(\""
        + assi
        + "\")"
        + "   }	@label(\"CSEifEnd\")"
        + "   return count;	@label(\"CSEreturn\")"
        + "}	@label(\"CSEend\") @closeContext"
        + "private int[] swap(int[] array , int index1 , int index2) {	@label(\"swap\") @openContext"
        + "   int tmp = array[index1];	@label(\"swapfirst\")"
        + "   array[index1] = array[index1 + index2];	@label(\"swapsecond\")"
        + "   array[index1 + index2] = tmp;	@label(\"swapthird\")"
        + "   return array;	@label(\"swapreturn\")"
        + "}	@label(\"swapEnd\") @closeContext";
  }
}
