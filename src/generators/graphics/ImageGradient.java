...
public class ImageGradient implements ValidatingGenerator {
    ...
    private ImageGradient imageGradientGen;

    public ImageGradient(Locale location) {
    	this.location = location;
    	this.translator = new Translator("resources/ImageGradient", location);
    }
    
    public void init(){
        lang = new AnimalScript("Image Gradient", "Marko Rücker", 1280, 1024);
    ...
    public String generate(AnimationPropertiesContainer props,Hashtable<String, Object> primitives) {  	
        ...
        header = lang.newText(new Coordinates(20,20), "Image Gradient (Prewitt Operator) - Step 1", "header",	null, headerProp);
        ...
        header.setText("      Image Gradient (Prewitt Operator)", null, null);
        ...
        // First Page
        header.setText("Image Gradient (Prewitt Operator) - Step 1", null, null);
        ...
        lang.nextStep(translator.translateMessage("LABEL_PREWITT_VERTICAL"));
        
        // Second Page (THE ALGORITHM)
        header.setText("Image Gradient (Prewitt Operator) - Step 2", null, null);
		kernelXmatrixAnim = lang.newIntMatrix(new Coordinates(880, 40), kernelX, "kernel_X_Matrix", null, matrixProp);
		kernelYmatrixAnim = lang.newIntMatrix(new Coordinates(880, 40), kernelY, "kernel_Y_Matrix", null, matrixProp);
		...
		dstTitleY.hide();
		kernelTitle = lang.newText(new Coordinates(880, 20), "Kernel Matrix: K_x", "kernelTitle", null, textPropTitle);
        ...
        lang.nextStep(translator.translateMessage("LABEL_PREWITT_HORIZONTAL"));
        scConvolute.hide();
        scApplyFilter.hide();
        
        // Third Page
        header.setText("Image Gradient (Prewitt Operator) - Step 3", null, null);
        createScThirdPage();
        ...
        // Fourth Page
        header.setText("Image Gradient (Prewitt Operator) - Step 4", null, null);
    ...
    public String getName() {
        return "Image Gradient (Prewitt-Operator) ["+location.toString()+"]";
    }

    public String getAlgorithmName() {
        return "Image Gradient (Prewitt-Operator)";
    }
	...
	public void convolute(IntMatrix src, IntMatrix kernel, IntMatrix dst, boolean autoRun) {				
		for(int i=1; i<src.getNrCols()-1; i++ ) {
			for(int j=1; j<src.getNrRows()-1; j++) {
				...
				dst.put(i-1, j-1, applyFilter(src, kernel, i-1, j-1), null, null);
	...
	private int euklidHelper(int v1, int v2) {
		...
		return (int)Math.round(Math.sqrt((double)(v1*v1)+(v2*v2)));
	}
