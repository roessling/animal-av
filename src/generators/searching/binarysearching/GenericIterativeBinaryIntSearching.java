...
public class GenericIterativeBinaryIntSearching extends
  ...
  protected int search(int valueToSearchFor) {
    ...
    mcq.addAnswer(
        "Das Suchfeld muss sortiert sein",
        2,
        "Ohne vorherige Sortierung findet die Bin&auml;re Suche nur zuf&auml;llig das richtige Element");
    mcq.addAnswer("Das Suchfeld darf nicht mehr als 1024 Elemente beinhalten",
        -1, "Die Gr&ouml;&szlig;e des Suchraums ist nicht begrenzt");
    mcq.addAnswer(
        "Das Suchfeld muss eine gemischte Folge von Zahlen und Buchstaben enthalten",
        1,
    ...
    FillInBlanksQuestionModel fib2 = f.generateFIBQuestion("nrComparisons",
        "AnzahlVergleiche");
    ...
    while (rMarker.getPosition() > lMarker.getPosition()
      ...
      mcq2.addAnswer("" + array.getData(midMarker.getPosition()), 1,
          "Das ist korrekt");
      if (midMarker.getPosition() == array.getLength() - 1) {
        mcq2.addAnswer(
            "" + array.getData(midMarker.getPosition() - 2),
            -1,
            "Das n&auml;chste mittlere Element lautet: "
                + array.getData(midMarker.getPosition()));
      } else {
        mcq2.addAnswer(
            "" + array.getData(midMarker.getPosition() + 1),
            -1,
            "Das n&auml;chste mittlere Element lautet: "
                + array.getData(midMarker.getPosition()));
      }
    ...
    fib.addAnswer("" + iteration, 5, "Die korrekte Antwort lautet: "
        + iteration);
    fib2.addAnswer("" + nrComparisons, 5, "Die korrekte Antwort lautet: "
        + nrComparisons);
