 ...
 * 
 * @author David Sharma, Björn Pantel
 * 
 */
public class KMP implements ValidatingGenerator {
  ...
  public void showSourceCodePhase2() {
    ...
    sc.addCodeLine("int i = 0;", null, 1, null);
    sc.addCodeLine("int j = 0;", null, 1, null);
    sc.addCodeLine("while (i < text.length) {", null, 1, null);
    sc.addCodeLine("while(j>=0 && !text[i].equals(muster[j])) {", null, 2, null);
    sc.addCodeLine("j = prefixTabelle[j];", null, 3, null);
    sc.addCodeLine("}", null, 2, null);
    sc.addCodeLine("i++;", null, 2, null);
    sc.addCodeLine("j++;", null, 2, null);
  ...
  public void findPattern() {
    ...
    while (i < text.length) {
      ...
      sc.toggleHighlight(10, 3);
      lang.nextStep();

    }
    if (i == text.length && j != pattern.length) {
      sc.toggleHighlight(3, 13);
  ...
  public String getCodeExample() {
    return "public void runKMP(String muster) { <br>" + " int i = 0;<br>"
        ...
        + "      System.out.println(''Muster wurde gefunden'');<br>"
        + "   }<br>" + " }<br>"
        + " if (i == text.length && j != muster.length) {<br>"
        + "    System.out.println(''Muster nicht gefunden'');<br>" + " }<br>"
        + "}<br>";
  }
  ...
  public String getDescription() {
    return "Der Knuth-Morris-Pratt Algorithmus ist ein String-Matching Algorithmus. "
        + "Ein Vorteil zur naiven Suche ist, dass wiederholende Vergleiche im Algorithmus vermieden werden."
        + "Der Algorithmus teilt sich in zwei Phasen auf. Die erste Phase nennt man 'Praefix-Analyse', die zweite Phase ist der eigentliche Sting-Matching Algorithmus."
        + "In der Praefix-Analyse wird eine Praefix Tabelle erstellt, die fuer Teilworte des gesuchten Musters die Laenge des Randes enthaelt."
        + "Der Rand eines Wortes w ist ein Teilwort w', das gleichzeitig (echtes) Praefix und (echtes) Suffix von w ist."
  ...
  public boolean validateInput(AnimationPropertiesContainer props,
    ...
    /*
     * if (t == null) return
     * "Der Eingabetext muss mit der Enter-Taste bestaetigt werden."; if (pat ==
     * null) return
     * "Der Mustertext muss mit der Enter-Taste bestaetigt werden."; if
     * (pat.length() > t.length()) return
     * "Das Mustertext muss kleiner oder gleich dem Eingabetext sein."; return
     * null;
     */
    if (t == null)
    ...
    if (pat.length() > t.length())
      throw new IllegalArgumentException(
          "Der Mustertext darf nicht laenger als der Eingabetext sein.");
    return true;
  }
