...
public class SequentielleSuche {
  ...
  // private boolean intSearch = true; //ggf. später StringSearch??
  ...
  private void displaySortingScreen() {
    ...
    if (!javaCode) {
      at.advancedCodeGroupStandard(
          algorithmName,
          new Off(15, 75, arrayName, "SW"),
          new TimeOffset(0),
          new String[] { "1. Beginne die Suche beim ersten Element des Arrays",
    ...
    if (linesInAlgorithm == null && linesInJavaCode == null) {
      ...
      if (linesInAlgorithm != null && !javaCode) {
        for (int i = 0; i < markedLinesInAlgorithm.length; i++) {
          at.unhighlightCode(algorithmName, markedLinesInAlgorithm[i], "", null);
        }
