...
public class SRTFgenerator implements Generator {
  ...
  public SRTFgenerator(Language language, SRTFgenerator hp) {
    this.lang = language;
  ...
  public void init() {
    lang = new AnimalScript(
        "Shortest-Remaining-Time-First Scheduling with Interrupts",
  ...
  public String generate(AnimationPropertiesContainer props,
    ...
    // System.out.println("x= "+inputProcesses[0].length);
    // System.out.println("y= "+inputProcesses.length);
    // System.out.println("[y][x]=[1][2]=28= "+ inputProcesses[1][2]);
    ...
    SRTFgenerator pr = new SRTFgenerator(lang, this);
  ...
  public void startScheduling(int[][] a, int x, int y) {
    ...
    lang.newText(new Coordinates(20, 30),
        "Shortest-Remaining-Time-First Scheduling with Interrupts", "header",
        null, headerProps);
    ...
    textProps3.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.BLACK);

    Text descr1 = lang.newText(new Coordinates(10, 150),
        "Shortest-Remaining-Time-First Scheduling with Interrupts is used to ",
    ...
    Text descr5 = lang.newText(new Offset(0, 25, "description4",
        AnimalScript.DIRECTION_NW),
        "The process with shortest remaining time is ", "description5", null,
        textProps3);
...
//    ArrayList<Text> te = new ArrayList<Text>();
    ...
    // +"\n7		2	}"
    // +"\n8   	2  	else{"
    // +"\n9	  	3   	chose the process with shortest remaining time"
    ...
    // +"\n14		1}"
    // +"\n15	}";
    sc.addCodeLine("HPF-Scheduling(){", null, 0, null); // 0
    ...
    sc.addCodeLine("chose the process with shortest remaining time", null, 3,
        null); // 8
    ...
    lang.newText(new Offset(550, 60, "stringArray" + (y - 1),
        AnimalScript.DIRECTION_NW), "Process\t  Arrival\t  Duration",
        "taskLine0", null, textProps);
    ...
    for (int i = 0; i < y; i++) {
      ...
      // //lang.newText(new Coordinates(600+70*2, (y-5)*25+390+40*i),
      // lang.newText(new Offset(210,
      // 30+30*i,"taskLine0",AnimalScript.DIRECTION_NW),
      // ""+a[i][3],
      // "taskCell0"+(i+1)+"_0", null, textProp);
    ...
    // textProps4.set(AnimationPropertiesKeys.FONT_PROPERTY, new Font(
    ...
    // Text summ6=lang.newText(new Offset(0, 25, "summary5",
    // AnimalScript.DIRECTION_NW),
    // "immediately executed,while other processes should wait.",
    // "summary6", null, textProps4);
  ...
  private void sheduling(ArrayList<StringArray> na, ArrayList<StringArray> na2,
    ...
    if (interaction) {
      String str[] = { "No, not really.", "No, not really.", "No, not really.",
          ...
          "No, not really." };
      if (lessTimeInd > -1)
        str[lessTimeInd] = "Absolutely right!";
    ...
    // +"\n8   	2  	else{"
      ...
      na.get(lessTimeInd).highlightCell(0, null, null);
      na2.get(lessTimeInd).highlightCell(0, null, null);
      // +"\n10		3 		set the status of the chosen process on 'working'"
      lang.nextStep();
      codeSupport.unhighlight(8, 0, false);
      codeSupport.highlight(9, 0, false);
      sa.get(lessTimeInd).highlightCell(step - 1, null, null);
      execTime++;
      ...
      for (int i = 0; i < y; i++) {
        if (i != lessTimeInd && a[i][1] < step && a[i][2] > 0) {
          if (!checked) {
            lang.nextStep();
