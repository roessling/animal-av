...
public class DiskschedulingFCFS implements ValidatingGenerator {
    ...
    //   new fcfs("resources/DiskschedulingFCFS", Locale.GERMANY);
    //or new fcfs("resources/DiskschedulingFCFS", Locale.US);
    public DiskschedulingFCFS(String spe, Locale loc){
        translator = new Translator(spe, loc);
    }
    
    public void init(){
    	lang = new AnimalScript("Disk Scheduling: FCFS", "Jana Vatter, Rene Hammann", 800, 600);
    ...
    public static void main(String[] args){
    	Generator generator = new DiskschedulingFCFS("resources/DiskschedulingFCFS",Locale.US);
	...
	public void schedule(int[] a, int s){
	    ...
	    // queue: ...
	    lang.newText(new Offset(0, 60, "head", AnimalScript.DIRECTION_NW),translator.translateMessage("arr3"), "queue", null, textProps);
	    ...
	    IntArray ia = lang.newIntArray(new Offset(20, 0, "queue", AnimalScript.DIRECTION_NE), a, "intArray",
	        null, arrayProps);
    	...
    	// dynamically create the labels for the box
        ...
        ArrayMarker marker = lang.newArrayMarker(ia, 0, "", null, arrayMProps);
	    ...
	    for(int i = 0; i<a.length; i++){
	    	...
	    	lang.nextStep("Request: "+String.valueOf(a[i]));
	    	
	    	code.highlight(2, 0, false);
	    	code.unhighlight(1, 0, false);
	    	ia.highlightElem(i, null, null);
	    	
	    	lang.newCircle(new Offset(a[i]*3, 40+cnt, "line", AnimalScript.DIRECTION_NW), 4, 
	    			"circle" + String.valueOf(a[i]), null, cProps);
	    	
	    	if(i>0 & i<a.length){
	    		node = new Node[]{new Offset(0, 0, "circle" + String.valueOf(a[i-1]), AnimalScript.DIRECTION_C),
		    			new Offset(0, 0, "circle" + String.valueOf(a[i]), AnimalScript.DIRECTION_C)};
	    	...
	    	if(i<a.length-1){
		    	...
		    	ia.unhighlightCell(i, null, null);
	    ...
	    ia.unhighlightCell(a.length-1, null, null);
	    marker.hide();
	    ...
	    sum.addCodeLine(translator.translateMessage("sum5"), null, 0, null);
