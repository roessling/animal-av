...
public class ShakerSortAnimation extends AnimatedIntArrayAlgorithm
  ...
  private ArrayProperties       arrProp;

  private ArrayMarkerProperties linksZeiger;
  private ArrayMarkerProperties rechtsZeiger;
  private ArrayMarkerProperties iZeiger;
  private ArrayMarkerProperties jZeiger;

  private SourceCodeProperties  scProp;

  private TextProperties        tp1;

  private TextProperties        tp2;

  private TextProperties        tp3;

  private TextProperties        tp4;
  ...
  public void sort(int[] liste) {
    ...
    Text t0 = language.newText(new Coordinates(150, 20), "SHAKERSORT",
        "f1" + textCounter, null, tp1);
    textCounter++;
    language.nextStep();
    ...
    Text t2 = language.newText(new Coordinates(20, 120),
        "1. Die zu sortierende Liste wird abwechselnd vom Beginn bzw. vom Ende her durchlaufen",
        "f1" + textCounter, null, tp3);
    textCounter++;
    language.nextStep();

    Text t3 = language.newText(new Coordinates(20, 160),
        "2. Vom Beginn her vergleicht man immer zwei benachbarte Elemente ",
        "f1" + textCounter, null, tp3);
    textCounter++;
    language.nextStep();

    Text t4 = language.newText(new Coordinates(20, 200),
        "3. Sind diese Elemente falsch sortiert, dann vertauscht man die",
        "f1" + textCounter, null, tp3);
    textCounter++;
    language.nextStep();

    Text t5 = language.newText(new Coordinates(20, 240),
        "4. Nach dem ersten Durchlauf ist das größte Element an seiner richtigen Position",
        "f1" + textCounter, null, tp3);
    textCounter++;
    language.nextStep();

    Text t6 = language.newText(new Coordinates(20, 280),
        "5. Danach geht man vom Ende her und vergleicht wiederum immer zwei benachbarte Elemente",
        "f1" + textCounter, null, tp3);
    textCounter++;
    language.nextStep();

    Text t7 = language.newText(new Coordinates(20, 320),
        "6. Sind diese falsch sortiert, dann vertauscht man die",
        "f1" + textCounter, null, tp3);
    textCounter++;
    language.nextStep();

    Text t8 = language.newText(new Coordinates(20, 360),
        "7. Nach dem zweiten Durchlauf, ist das kleinste Elemente an seiner richtigen Position",
        "f1" + textCounter, null, tp3);
    textCounter++;
    language.nextStep();

    Text t9 = language.newText(new Coordinates(20, 400),
        "8. Die Schritte 2 bis 7 wiederholt man bis alle Elemente sortiert sind.",
        "f1" + textCounter, null, tp3);
    textCounter++;
    language.nextStep();
    ...
    language.newText(new Coordinates(150, 20), "ShakerSort Animationsbeispiel",
        "f1" + textCounter, null, tp4);

    // IntArray-Objekt erzeugt, welches auf die Display-Eigenschaften verweist
    IntArray array = language.newIntArray(new Coordinates(20, 100), liste,
        "array", null, arrProp);

    // SourceCode Objekt erzeugen
    SourceCode sc = language.newSourceCode(new Coordinates(20, 150), "code", null, scProp);

    // Zeilen im SourceCode hinzufügen
    sc.addCodeLine(
        "public <T extends Comparable <T>> void BubleSort (T [] liste)", null,
        0, null);
  ...
  public void shakerSort(IntArray array, SourceCode sourceCode) {
    ...
    ArrayMarker linksPointer = language.newArrayMarker(array, 0, "links", null,
        linksZeiger);

    language.nextStep();
    sourceCode.toggleHighlight(2, 0, false, 3, 0);

    ArrayMarker rechtsPointer = language.newArrayMarker(array, rechts, "rechts",
        null, rechtsZeiger);
    ...
    while (vertauscht) {
      ...
      ArrayMarker iPointer = language.newArrayMarker(array, links, "i", null,
          iZeiger);
      for (i = links; i < rechts; i++) {
        ...
        if (array.getData(i) > array.getData(i + 1)) {
          ...
          iPointer.move(i, new TicksTiming(0), new TicksTiming(50));

        } else {
          language.nextStep();
          sourceCode.toggleHighlight(10, 0, false, 8, 0);
          iPointer.move(i + 1, new TicksTiming(0), new TicksTiming(50));
        }
      ...
      rechtsPointer.move(rechts, new TicksTiming(0), new TicksTiming(50));

      language.nextStep();
      sourceCode.toggleHighlight(16, 0, false, 17, 0);

      ArrayMarker jPointer = language.newArrayMarker(array, rechts, "j", null,
          jZeiger);
      ...
      for (j = rechts; j > links; j--) {
        ...
        if (array.getData(j) < array.getData(j - 1)) {
          ...
          jPointer.move(j, new TicksTiming(0), new TicksTiming(50));
        } else {
          language.nextStep();
          sourceCode.toggleHighlight(19, 0, false, 17, 0);
          jPointer.move(j - 1, new TicksTiming(0), new TicksTiming(50));
        }
      ...
      linksPointer.move(links, new TicksTiming(0), new TicksTiming(50));
    ...
    tp.set(AnimationPropertiesKeys.FONT_PROPERTY,
        new Font(DEFAULT_FONT_NAME, Font.BOLD, 40));
  ...
  public static void main(String[] args) {
    ...
    System.err.println(shakerSort.language.toString());
  }
  ...
  public void init() {
    ...
    iZeiger = new ArrayMarkerProperties();
    iZeiger.set(AnimationPropertiesKeys.LABEL_PROPERTY, "i");
    iZeiger.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.blue);

    jZeiger = new ArrayMarkerProperties();
    jZeiger.set(AnimationPropertiesKeys.LABEL_PROPERTY, "j");
    jZeiger.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.blue);

    linksZeiger = new ArrayMarkerProperties();
    linksZeiger.set(AnimationPropertiesKeys.LABEL_PROPERTY, "links");
    linksZeiger.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.pink);

    rechtsZeiger = new ArrayMarkerProperties();
    rechtsZeiger.set(AnimationPropertiesKeys.LABEL_PROPERTY, "rechts");
    rechtsZeiger.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.pink);

    tp1 = new TextProperties();
    tp1.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.blue);
    tp1.set(AnimationPropertiesKeys.FONT_PROPERTY,
        new Font(DEFAULT_FONT_NAME, Font.BOLD, 35));

    tp2 = new TextProperties();
    tp2.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.black);
    tp2.set(AnimationPropertiesKeys.FONT_PROPERTY,
        new Font(DEFAULT_FONT_NAME, Font.BOLD, 24));

    tp3 = new TextProperties();
    tp3.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.black);
    tp3.set(AnimationPropertiesKeys.FONT_PROPERTY,
        new Font(DEFAULT_FONT_NAME, Font.PLAIN, 20));

    tp4 = new TextProperties();
    tp4.set(AnimationPropertiesKeys.COLOR_PROPERTY, Color.black);
    tp4.set(AnimationPropertiesKeys.FONT_PROPERTY,
        new Font(DEFAULT_FONT_NAME, Font.BOLD, 35));

    scProp = new SourceCodeProperties();
    ...
    scProp.set(AnimationPropertiesKeys.CONTEXTCOLOR_PROPERTY, Color.cyan);
    scProp.set(AnimationPropertiesKeys.FONT_PROPERTY,
