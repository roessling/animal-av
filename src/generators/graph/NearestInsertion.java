...
public class NearestInsertion implements Generator {
    private Language lang;
    private SourceCodeProperties sourceCode;
    private Graph graph;

    public void init(){
        lang = new AnimalScript("Nearest Insertion", "Patricia Heidt", 800, 600);
    ...
    public String getDescription(){
        return "Nearest Insertion ist eine Heuristik, die auf das Problem " 
        		...
        		+ "\n"
        		+ "Der Ansatz von Nearest Insertion ist immer den Punkt, der "
        		+ "am k&uuml;rzesten entfernt ist, so in die bestehende Route "
        		+ "einzuf&uuml;gen,"
        		+ "dass sich die L&auml;nge der Route nur minimal erh&ouml;ht.\n"
        		+ "Am k&uuml;rzesten entfernt bedeutet hier, dass er die "
        		+ "geringste Entfernung zu dem von ihm aus n&auml;chsten "
        		+ "bereits verbundenen"
        		+ "Knoten hat.";
    }

    public String getCodeExample(){
        return "1. Zuf&auml;lligen Knoten w&auml;hlen\n"
        		+ "2. Knoten finden, der k&uuml;rzeste Distanz zum "
        		+ "n&auml;chsten Routenknoten hat\n"
	...
	private static final String[] DESCRIPTION = {"Nearest Insertion ist eine Heuristik "
			+ "die auf das Problem des Handelsreisenden angewendet werden kann.",
			"Es geht darum, die kürzeste Route zu finden, die alle gefragten "
			+ "Punkte erreicht und am gleichen Punkt endet, wo sie begonnen hat.",
			"",
			"Der Ansatz von Nearest Insertion ist immer den Punkt, der am kürzesten "
			+ "entfernt ist so in die bestehende Route einzufügen,",
			"dass sich die Länge der Route nur minimal erhöht.",
			"Am kürzesten entfernt bedeutet hier, dass er die geringste Entfernung zu "
			+ "dem von ihm aus nächsten bereits verbundenen Knoten hat."};
	
	private static final String[] INIT = {"Die hier gezeigten Knoten werden nun in den",
			"folgenden Schritten mithilfe der Nearest Insertion",
			"Heuristik verbunden."};
	
	private static final String[] PSEUDO_CODE = {"1. Zufälligen Knoten wählen",
			"2. Knoten finden, der kürzeste Distanz zum nächsten Routenknoten hat",
			"3. Diesen Knoten so einfügen, dass kürzester Weg entsteht",
			"4. Falls noch unverbundene Knoten übrig sind, zurück zu Schritt 2"};
	
	private String[] endText() {
		String[] text = {"In jedem Schritt wurde der am kürzesten"
			+ " entfernte Knoten ausgewählt und möglichst günstig",
			"in die bereits bestehende Route eingefügt.",
			"Hierfür wurden " + (nodeNum - 1) + " Iterationen benötigt.",
			"Die entstandene Route hat eine Länge von " + String.format("%.2f", completeDistance) + ".",
			"Zum Vergleich, eine Route, in die die Knoten in zufälliger Reihenfolge aneinandergereit sind,",
			"hätte beispielsweise eine Länge von " + String.format("%.2f", randomDistance) + ".",
			"Auch wenn die kürzeste Verbindung in diesem Beispiel vielleicht offensichtlich war, ist das,",
	...
	public NearestInsertion(Language l) {
		lang = l;
		lang.setStepMode(true);
	}
	...
	private void initialize() {
		...
		Text header = lang.newText(new Coordinates(30,30), "Nearest Insertion", "header", null, headerProp);
		...
		String[] legendText = {"wurde noch nicht bearbeitet:",
				...
				"wird als nächstes eingefügt:",
				"ist Teil der Route:",
				"wird im nächsten Schritt ersetzt:"};
	...
	/**
	 * Finds the nearest node that is not selected yet.
	 * @param graph the graph
	 * @param linked the indices of the nodes which already are linked
	 * @return the node which is nearest to the nodes already linked
	 */
	private Coordinates findNearest(Graph graph, Set<Integer> linked) {
		int size = graph.getSize();
		Coordinates nearestNode = null;
		...
		for (int i = 0; i < size; i++) {
			...
			if (shortestDistance < nearestDistance) {
				nearestDistance = shortestDistance;
				nearestNode = node;
			}
		}
		return nearestNode;
	 ...
	 * cause the lowest additional distance.
	...
	/**
	 * Performs the nearest insertion algorithm to link all nodes of the graph
	 */
	private void link() {
		lang.nextStep();
		...
		while (linked.size() < nodeNum) {
			lang.nextStep();
			Coordinates newNode = findNearest(graph, linked);
