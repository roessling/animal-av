...
public class BidirectionalDijkstra2 implements Generator {
	...
	public BidirectionalDijkstra2(Language lang) {
		this.lang = lang;
	...
	public BidirectionalDijkstra2() {
		this.lang = new AnimalScript("Direrektionlaer Dijkstra", "Marian Hieke, Tim Beringer", 800, 600);
	...
	public String generate(AnimationPropertiesContainer props, Hashtable<String, Object> primitives) {
		...
		// .println("blub2");
		// this.srcProps =
		// (SourceCodeProperties)props.getPropertiesByName("sourceCode"); //<------
		// error solved in xml
		...
		/*
		 * graph = getDefaultGraph(graphProps); .print(graph.getSize()); for (int i = 0;
		 * i < matrix.length; i++) { for (int j = 0; j < matrix[0].length; j++) {
		 * .print(matrix[i][j] + "\t"); } .println(); }
		 */
		/*
		 * int size = graph.getSize(); Node[] nodes = new Node[size]; String[]
		 * nodeLabels = new String[size]; for (int i = 0; i < size; i++) { nodes[i] =
		 * graph.getNode(i); nodeLabels[i] = graph.getNodeLabel(i);
		 * 
		 * graph = lang.newGraph(graph.getName(), graph.getAdjacencyMatrix(), nodes,
		 * nodeLabels, graph.getDisplayOptions(), graphProps);
		 */
	...
	public String getDescription() {
		...
		description = description
				+ "Der Bidirektionale Dijkstra setzt sich aus zwei unidirektional verlaufende Dijkstra-Suchen zusammen. Eine Suche beginnt von Start- zu Zielknoten w�hrend die zweite von Ziel- zu Startknoten sucht. \r\n";
		description = description
				+ "In dieser Implementierung alternieren die beiden Suchen in jeder Iteration, was ohne Verlust der Optimalit�t (Finden des k�rzesten Weges) auch durch eine beliebig andere Strategie ersetzt werden kann. Motivation der bidirektionalen Suche ist die Verk�rzung der Laufzeit, in dem beide Suchen sich im optimalen Fall bei der H�lfte des Weges treffen, sodass die Anzahl der zu expandierenden Knoten stark verringert wird. Nachdem sich die beiden Suchen treffen, m�ssen in einer zweiten Phase, der so genannten Postprocessing Phase, weitere Alternativrouten, die potentiell k�rzer sind, �berpr�ft werden.";

		return description;
	}

	public String getCodeExample() {
		String description = "// Vorw�rts \r\n" + "Initialisiere f�r alle Knoten v ds(v) := +oo \r\n"
				+ "Setze ds(s) := 0;\r\n" + "F�r alle Folgeknoten v von s mit a=(s,v):\r\n"

				+ "ds(v) := l(a)\r\n" + "F�ge alle Knoten au�er s in Qs.\r\n" + "Solange Qs nicht leer ist:\r\n"
				+ "  Entferne n�chsten Knoten v aus Qs.\r\n" + "  F�r alle ausgehenden Kanten a=(v,w) v:\r\n"
				...
				+ "  Entferne n�chsten Knoten v aus Qg.\r\n" + "  F�r alle ausgehenden Kanten a=(v,w) v:\r\n"
				+ "    Wenn w in Qg liegt:\r\n" + "      Setze ds(w) := min{ ds(w), ds(v) + l(a) }\r\n"
				+ "Wenn v nicht in Qs ist:\r\n" + "Gehe zu Phase 2\r\n" + "\r\n" + "// Phase 2:\r\n" + "L := oo\r\n"
				+ "F�r alle Kanten (x,y) mit x nicht in Qs und y nicht in Qg:\r\n"
				+ "  L := min { L, ds(x) + l(x,y) + dg(y) }\r\n" + "K�rzeste Distanz von Start zu Ziel = L\r\n";
	...
	public void start(Graph graph, GraphProperties graphProps) {
		...
		lang.newText(new Coordinates(10, 90),
				"Der Bidirektionale Dijkstra setzt sich aus zwei unidirektional verlaufende Dijkstra-Suchen zusammen.",
				"descript1", null, textprops);
		lang.newText(new Coordinates(10, 120),
				"Eine Suche beginnt von Start- zu Zielknoten w�hrend die zweite von Ziel- zu Startknoten sucht.",
				"descript2", null, textprops);
		lang.newText(new Coordinates(10, 140),
				"In dieser Implementierung alternieren die beiden Suchen in jeder Iteration,", "descript3", null,
				textprops);
		lang.newText(new Coordinates(10, 160),
				"was ohne Verlust der Optimalit�t (Finden des k�rzesten Weges) auch durch eine beliebig andere Strategie ersetzt werden kann.",
				"descript4", null, textprops);
		lang.newText(new Coordinates(10, 180),
				"Motivation der bidirektionalen Suche ist die Verk�rzung der Laufzeit, in dem beide Suchen sich im optimalen Fall bei der H�lfte des Weges treffen,",
				"descript5", null, textprops);
		lang.newText(new Coordinates(10, 200), "sodass die Anzahl der zu expandierenden Knoten stark verringert wird.",
				"descript6", null, textprops);
		lang.newText(new Coordinates(10, 220),
				"Nachdem sich die beiden Suchen treffen, m�ssen in einer zweiten Phase, der so genannten Postprocessing Phase,",
				"descript7", null, textprops);
		lang.newText(new Coordinates(10, 240),
				"weitere Alternativrouten, die potentiell k�rzer sind, �berpr�ft werden.", "descript8", null,
				textprops);
		...
		header.show();
		src = lang.newSourceCode(new Coordinates(10, 360), "sourceCode", null, srcProps);
		src.addCodeLine("// Vorw�rts", null, 0, null); // 0
		src.addCodeLine("Initialisiere f�r alle Knoten v ds(v) := +oo", null, 1, null);
		src.addCodeLine("Setze ds(s) := 0;", null, 1, null);
		src.addCodeLine("F�r alle Folgeknoten v von s mit a=(s,v):", null, 1, null);

		src.addCodeLine("ds(v) := l(a)", null, 1, null);
		src.addCodeLine("F�ge alle Knoten au�er s in Qs.", null, 1, null);
		...
		src.addCodeLine("F�r alle ausgehenden Kanten a=(v,w) v:", null, 2, null);
		...
		src.hide();
		src2 = lang.newSourceCode(new Coordinates(550, 360), "sourceCode", null, srcProps);
		src2.addCodeLine("// R�ckw�rts", null, 0, null); // 0
		src2.addCodeLine("Initialisiere f�r alle Knoten v ds(v) := +oo", null, 1, null);
		src2.addCodeLine("Setze ds(s) := 0;", null, 1, null);
		src2.addCodeLine("F�r alle Folgeknoten v von s mit a=(s,v):", null, 1, null);

		src2.addCodeLine("ds(v) := l(a)", null, 1, null);
		src2.addCodeLine("F�ge alle Knoten au�er s in Qs.", null, 1, null);
		...
		src2.addCodeLine("F�r alle ausgehenden Kanten a=(v,w) v:", null, 2, null);
		...
		src3.addCodeLine("// Phase 2:", null, 0, null); // 0
		src3.addCodeLine("L := oo", null, 0, null);
		src3.addCodeLine("F�r alle Kanten (x,y) mit x nicht in Qs und y nicht in Qg:", null, 0, null);

		src3.addCodeLine("L := min { L, ds(x) + l(x,y) + dg(y) }", null, 1, null);

		src3.addCodeLine("K�rzeste Distanz von Start zu Ziel = L", null, 0, null);
		...
		// Graph newGraph = lang.newGraph(graph.getName(), graph.getAdjacencyMatrix(),
		// getNodes(graph), getLabels(graph), graph.getDisplayOptions(),
		// graphProps, 500, 30, 800, 330, 40, 20, 100, 10, true, 20, 20);
		// Graph newGraph = lang.newGraph(graph.getName(),
		// graph.getAdjacencyMatrix(),getNodes(graph), getLabels(graph),
		// graph.getDisplayOptions(), graphProps,500, 30, 800, 330);

		// Graph newGraph = lang.newGraph(graph.getName(), graph.getAdjacencyMatrix(),
		// getLabels(graph), graph.getDisplayOptions(), graphProps, 500, 30, 800,
		// 330, 40, 20, 100, 10, true, 20, 20);

		Graph newGraph = lang.newGraph(graph.getName(), graph.getAdjacencyMatrix(), getLabels(graph),
				graph.getDisplayOptions(), graphProps, 500, 30, 800, 330);
		// Graph newGraph = lang.newGraph(graph.getName(),
		// graph.getAdjacencyMatrix(),getNodes(graph), getLabels(graph),
		// graph.getDisplayOptions(), graphProps);
		newGraph.setStartNode(graph.getNodeLabel(graph.getStartNode()));
		newGraph.setTargetNode(graph.getNodeLabel(graph.getTargetNode()));
	...
	private void biDijk(Graph graph, StringMatrix dist, StringMatrix info, ArrayProperties arrayProps, String[] data) {
		...
		src2.show();
		src3.hide();
		Text nameDist = this.lang.newText(new Coordinates(5, 110), dist.getName(), dist.getName(), null,
				this.textprops);
		nameDist.show();
		...
		Text nameQueueS = this.lang.newText(new Coordinates(5, 50), queueS.getName(), queueS.getName(), null,
				this.textprops);
		nameDist.show();
		Text nameQueueG = this.lang.newText(new Coordinates(5, 80), queueG.getName(), queueG.getName(), null,
				this.textprops);
		nameDist.show();
		...
		Text nameTime = this.lang.newText(new Coordinates(5, 270), time.getName(), time.getName(), null,
				this.textprops);
		...
		graph.highlightNode(j, null, null);
		src3.unhighlight(4);
		lang.newText(new Coordinates(10, 480),
				"Da die beiden unidirektionalen Suchen aufeinander getroffen sind und weiterhin s�mtliche ",
				"enddescript1", null, textprops);
		lang.newText(new Coordinates(10, 500),
				"Alternativrouten in der zweiten Phase untersucht wurden, ist mit L die Distanz des k�rzesten Pfades gefunden.",
				"enddescript2", null, textprops);
	...
	// returns Index the lowest Node represented in the "Queue" (queue Index =
	// graph Index )
	...
	public void addCost(ArrayList<Integer> children, Graph graph, int start, StringMatrix dist, StringArray queue,
			int row, SourceCode source, int way[]) {
		for (int i = 0; i < children.size(); i++) {
			lang.nextStep();
