...
public class ColorChooserComboBox extends JComboBox implements ActionListener {
    ...
    private static final String[] strAnimalNames = { "black", "dark Gray",
            ...
            "light_blue", "green4", "green3", "green2", "cyan4", "cyan3",
            "cyan2", "red4", "red3", "red2", "magenta4", "magenta3",
            "magenta2", "brown4", "brown3", "brown2", "pink4", "pink3",
            "pink2", "gold" };

    /** stores the Keys for the Translator */
    private static final String[] strTranslatorKeys = { "black", "darkGray",
            "gray", "lightGray", "white", "red", "pink", "orange", "yellow",
            "green", "cyan", "blue", "magenta", "blue4", "blue3", "blue2",
            "light_blue", "green4", "green3", "green2", "cyan4", "cyan3",
            "cyan2", "red4", "red3", "red2", "magenta4", "magenta3",
            "magenta2", "brown4", "brown3", "brown2", "pink4", "pink3",
            "pink2", "gold", "moreColors", "customColor", "colorChooserTitle" };

    /** stores the Color values for the Animal Colors. */
    static final Color[] colorValues = { Color.black, Color.darkGray,
            Color.gray, Color.lightGray, Color.white, Color.red, Color.pink,
            Color.orange, Color.yellow, Color.green, Color.cyan, Color.blue,
            Color.magenta, new Color(0, 0, 0x90), new Color(0, 0, 0xb0),
            new Color(0, 0, 0xd0), new Color(0x87, 0xce, 0xff),
            new Color(0, 0x90, 0), new Color(0, 0xb0, 0),
            new Color(0, 0xd0, 0), new Color(0, 0x90, 0x90),
            new Color(0, 0xb0, 0xb0), new Color(0, 0xd0, 0xd0),
            new Color(0x90, 0, 0), new Color(0xb0, 0, 0),
            new Color(0xd0, 0, 0), new Color(0x90, 0, 0x90),
            new Color(0xb0, 0, 0xb0), new Color(0xd0, 0, 0xd0),
            new Color(0x80, 0x30, 0), new Color(0xa0, 0x40, 0),
            new Color(0xc0, 0x60, 0), new Color(0xff, 0x80, 0x80),
            new Color(0xff, 0xa0, 0xa0), new Color(0xff, 0xe0, 0xe0),
            new Color(0xff, 0xd7, 0) };
    ...
    static String[] strLocalStrings = { "Black", "Dark Gray", "Gray",
            ...
            "Cyan", "Blue", "Magenta", "Blue 4", "Blue 3", "Blue 2",
            "Light Blue", "Green 4", "Green 3", "Green 2", "Cyan 4", "Cyan 3",
            "Cyan 2", "Red 4", "Red 3", "Red 2", "Magenta 4", "Magenta 3",
            "Magenta 2", "Brown 4", "Brown 3", "Brown 2", "Pink 4", "Pink 3",
            "Pink 2", "Gold", "More Colors...", "Custom Color",
    ...
    /**
     * Constructor creates a new ColorChooserComboBox-Object and sets the
     * selected Color.
     * 
     * @param colorNew The default Color.
     */
    ...
    /**
     * Constructor creates a new ColorChooserComboBox-Object and sets the
     * selected Color via a String
     * 
     * @param strNewColor The default Color as an Animal-Color-String.
     */
     ...
     * 
     * @param colorNew The Color that should be selected.
     */
     ...
     * 
     * @param strNewColor The Color that should be selected as an
     * Animal-Color-String or an RGB String.
     */
    public void setColorSelected(String strNewColor) {
        if (strNewColor == null) {
            return;
        }
        String effectiveColorName = strNewColor.trim();

        /*
         * first we test if the String is a RGB String the patterns goes \x28 ->
         * "(" \d{1,3} ->1,2 or 3 digits, ,\s -> ", " \x29 -> ")"
         */
        if (effectiveColorName.matches("\\x28((\\d{1,3})(,\\s)){2}(\\d{1,3})\\x29")) {
            ...
            int fc = effectiveColorName.indexOf(", ");
            int sc = effectiveColorName.indexOf(", ", fc + 2);
            ...
            // now read the values
            try {
                r = Integer.parseInt(effectiveColorName.substring(1, fc));
                g = Integer.parseInt(effectiveColorName.substring(fc + 2, sc));
                b = Integer.parseInt(effectiveColorName.substring(sc + 2, effectiveColorName
                        .length() - 1));
            ...
            // we have an Animal Name String
            for (int i = 0; i < colorValues.length; i++) {
                if (strAnimalNames[i].equals(effectiveColorName)) {

                    // if the color is found within the Animal Colors, then
                    // this element is chosen
                    this.strSelected = effectiveColorName;
            ...
            // nothing is done if the String is not a valid Animal Color
            // String...
        }
    }
     ...
     * 
     * @param newLocalStrings The localized Strings that should be used.
     */
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (index < colorValues.length) {
            ...
            Color colorNew = JColorChooser
                    .showDialog(this, strLocalStrings[colorValues.length + 2],
                            this.colorSelected);
    ...
    /**
     * getStringForColorRGB returns the appropriate String for the new Color,
     * but doesn't look if an Animal Color exists that is like the Color.
     * 
     * @param colorObject The Color.
    ...
    /*
     * **************************************************** 
     * BELOW ARE TWO HELPER CLASSES
     * ****************************************************
     */

    /**
     * ColoredSquare is a Helper Class that makes creating the different Icons
     * for the ColorChooserComboBox easier.
     * 
     * @author T. Ackermann
     */
    private static class ColoredSquare implements Icon {
        ...
        /**
         * Constructor creates a new ColoredSquare-Object but because no Color
         * was passed a special Icon for the "More Colors..." Button is painted.
         */
         ...
         * java.awt.Graphics, int, int) paints the Icon. The special Icon has
         * three dots on it and all the other Icons are drawn in the specified
         * Color.
         */
        public void paintIcon(Component c, Graphics g, int x, int y) {
            ...
            if (!this.special) {
                ...
                g.fill3DRect(x + 2, y, getIconWidth() - 2, getIconHeight(),
                        true);
            } else {
     ...
     * ColoredSquare-Object and the next one is the text that should be
     * displayed.
     * 
     * @author T. Ackermann
     */
    private class ComboBoxRenderer extends JLabel implements ListCellRenderer {
        ...
        /**
         * a generated serial Version UID because ComboBoxRenderer is
         * serializable.
         */
        ...
        public Component getListCellRendererComponent(JList list, Object value,
            ...
            if ((index >= 0)
                    || (!this.getText()
                            .equals(strLocalStrings[colorValues.length]))) {
