...
/**
 * Editor for a JK flip flop
 * 
 * @author p_li
 */
public class JKEditor extends FillablePrimitiveEditor implements ItemListener,
	...
	protected void buildGUI() {
				...
				TitledBorder.LEADING, TitledBorder.TOP));
		{
			...
			Box inputBox = new Box(BoxLayout.PAGE_AXIS);
			{
				...
				{
					// set name label
					JLabel setLabel = new JLabel("J pin name: ");
					setBox.add(setLabel);
					// set name text
					setText = new JTextField("inJ");
				...
				{
					// reset name label
					JLabel resetLabel = new JLabel("K pin name: ");
					resetBox.add(resetLabel);
					// reset name text
					resetText = new JTextField("inK");
	...
	public boolean nextPoint(int num, Point p) {
		PTJK not = (PTJK) getCurrentObject();
		if (num == 1)
	...
	public void setProperties(XProperties props) {
		colorChooser.setColor(props.getColorProperty(
				PTJK.JK_FLIPFLOP_TYPE_LABEL + ".color", Color.black));
		depthBox.setSelectedItem(props.getProperty(PTJK.JK_FLIPFLOP_TYPE_LABEL + ".depth",
				"16"));
		fillColorChooser.setColor(props.getColorProperty(PTJK.JK_FLIPFLOP_TYPE_LABEL
				+ ".color", Color.black));
		filledCB
				.setSelected(props.getBoolProperty(PTJK.JK_FLIPFLOP_TYPE_LABEL + ".filled"));
	}

	public void getProperties(XProperties props) {
		props.put(PTJK.JK_FLIPFLOP_TYPE_LABEL + ".color", colorChooser.getColor());
		props.put(PTJK.JK_FLIPFLOP_TYPE_LABEL + ".depth", depthBox.getSelectedItem());
		props.put(PTJK.JK_FLIPFLOP_TYPE_LABEL + ".fillColor", fillColorChooser.getColor());
		props.put(PTJK.JK_FLIPFLOP_TYPE_LABEL + ".filled", filledCB.isSelected());
	}
	...
	public String getStatusLineMsg() {
		return AnimalTranslator.translateMessage("JKEditor.statusLine",
	...
	public void propertyChange(PropertyChangeEvent event) {
		PTJK poly = (PTJK) getCurrentObject();
	...
	private void setOutputValueOf(PTJK ff) {
		...
		LogicJK ljk = new LogicJK(inputList);
		char qv = booleanToChar(ljk.getQValue());
		char revQV = booleanToChar(ljk.getRevQValue());
