...
import animal.vhdl.logic.LogicOr;

/**
 * Editor for an or gate
 * 
 * @author p_li
 */
public class OrEditor extends FillablePrimitiveEditor implements ItemListener,
	...
	protected void buildGUI() {
		...
		ChangeListener listener = new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				PTOr gate = (PTOr) getCurrentObject();
				if (gate != null) {
	...
	public void setProperties(XProperties props) {
		colorChooser.setColor(props.getColorProperty(
				PTOr.OR_TYPE_LABEL + ".color", Color.black));
		depthBox.setSelectedItem(props.getProperty(PTOr.OR_TYPE_LABEL + ".depth",
				"16"));
		fillColorChooser.setColor(props.getColorProperty(PTOr.OR_TYPE_LABEL
				+ ".color", Color.black));
		filledCB
				.setSelected(props.getBoolProperty(PTOr.OR_TYPE_LABEL + ".filled"));
		// inputAmountSpinner.setValue(props.getIntProperty(PTOr.OR_TYPE_LABEL
		// + ".inputAmount", 2));
	}

	public void getProperties(XProperties props) {
		props.put(PTOr.OR_TYPE_LABEL + ".color", colorChooser.getColor());
		props.put(PTOr.OR_TYPE_LABEL + ".depth", depthBox.getSelectedItem());
		props.put(PTOr.OR_TYPE_LABEL + ".fillColor", fillColorChooser.getColor());
		props.put(PTOr.OR_TYPE_LABEL + ".filled", filledCB.isSelected());
		// props.put(PTOr.OR_TYPE_LABEL + ".inputAmount", inputAmountSpinner
		// .getValue().toString());
		// props.put(PTOr.OR_TYPE_LABEL + ".outputName", String.valueOf(outText
		// .getText()));
	}
	...
	public Editor getSecondaryEditor(EditableObject go) {
		...
		OrEditor result = new OrEditor();
		result.extractAttributesFrom(go);
		return result;
	}

	public String getStatusLineMsg() {
		return AnimalTranslator.translateMessage("OrEditor.statusLine",
	...
	private void calculateOutputValueOf(PTOr gate) {
		...
		LogicOr lor = new LogicOr(inputList);
		char outputValue = lor.getLogicResult();
