...
import animal.vhdl.logic.LogicNand;

/**
 * Editor for an nand gate
 * 
 * @author p_li
 */
public class NandEditor extends FillablePrimitiveEditor implements ItemListener,
	...
	protected void buildGUI() {
		...
		ChangeListener listener = new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				PTNand gate = (PTNand) getCurrentObject();
				if (gate != null) {
	...
	public void setProperties(XProperties props) {
		colorChooser.setColor(props.getColorProperty(PTNand.NAND_TYPE_LABEL
				+ ".color", Color.black));
		depthBox.setSelectedItem(props.getProperty(PTNand.NAND_TYPE_LABEL + ".depth",
				"16"));
		fillColorChooser.setColor(props.getColorProperty(PTNand.NAND_TYPE_LABEL
				+ ".color", Color.black));
		filledCB.setSelected(props
				.getBoolProperty(PTNand.NAND_TYPE_LABEL + ".filled"));
		// inputAmountSpinner.setValue(props.getIntProperty(PTNand.NAND_TYPE_LABEL
		// + ".inputAmount", 2));
	}

	public void getProperties(XProperties props) {
		props.put(PTNand.NAND_TYPE_LABEL + ".color", colorChooser.getColor());
		props.put(PTNand.NAND_TYPE_LABEL + ".depth", depthBox.getSelectedItem());
		props.put(PTNand.NAND_TYPE_LABEL + ".fillColor", fillColorChooser.getColor());
		props.put(PTNand.NAND_TYPE_LABEL + ".filled", filledCB.isSelected());
		// props.put(PTNand.NAND_TYPE_LABEL + ".inputAmount", inputAmountSpinner
		// .getValue().toString());
		// props.put(PTNand.NAND_TYPE_LABEL + ".outputName", String.valueOf(outText
		// .getText()));
	}
	...
	public Editor getSecondaryEditor(EditableObject go) {
		...
		NandEditor result = new NandEditor();
		result.extractAttributesFrom(go);
		return result;
	}

	public String getStatusLineMsg() {
		return AnimalTranslator.translateMessage("NandEditor.statusLine",
	...
	private void calculateOutputValueOf(PTNand gate) {
		...
		LogicNand lnand = new LogicNand(inputList);
		char outputValue = lnand.getLogicResult();
	...
	public void mouseClicked(MouseEvent me) {
		...
		if (gate != null) {
			if (gate.getInputPins() != null) {
				...
				inputNames = new ArrayList<JTextField>(gate.getInputPins().size());
				inputValues = new ArrayList<JComboBox>(gate.getInputPins().size());
				for (int i = 0; i < gate.getInputPins().size(); i++) {
					...
					newInputValueCombo.setSelectedItem(gate.getInputPins().get(i)
							.getPinValue());
