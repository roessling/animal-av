...
/**
 * Editor for an xor gate
 * 
 * @author p_li
 */
public class XorEditor extends FillablePrimitiveEditor implements ItemListener,
	...
	protected void buildGUI() {
		...
		ChangeListener listener = new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				PTXor gate = (PTXor) getCurrentObject();
				if (gate != null) {
	...
	public void setProperties(XProperties props) {
		colorChooser.setColor(props.getColorProperty(PTXor.XOR_TYPE_LABEL
				+ ".color", Color.black));
		depthBox.setSelectedItem(props.getProperty(PTXor.XOR_TYPE_LABEL + ".depth",
				"16"));
		fillColorChooser.setColor(props.getColorProperty(PTXor.XOR_TYPE_LABEL
				+ ".color", Color.black));
		filledCB.setSelected(props
				.getBoolProperty(PTXor.XOR_TYPE_LABEL + ".filled"));
		// inputAmountSpinner.setValue(props.getIntProperty(PTXor.TYPE_LABEL
		// + ".inputAmount", 2));
	}

	public void getProperties(XProperties props) {
		props.put(PTXor.XOR_TYPE_LABEL + ".color", colorChooser.getColor());
		props.put(PTXor.XOR_TYPE_LABEL + ".depth", depthBox.getSelectedItem());
		props.put(PTXor.XOR_TYPE_LABEL + ".fillColor", fillColorChooser.getColor());
		props.put(PTXor.XOR_TYPE_LABEL + ".filled", filledCB.isSelected());
		// props.put(PTXor.TYPE_LABEL + ".inputAmount", inputAmountSpinner
		// .getValue().toString());
		// props.put(PTXor.TYPE_LABEL + ".outputName", String.valueOf(outText
		// .getText()));
	}
	...
	public Editor getSecondaryEditor(EditableObject go) {
		...
		XorEditor result = new XorEditor();
		result.extractAttributesFrom(go);
		return result;
	}

	public String getStatusLineMsg() {
		return AnimalTranslator.translateMessage("XorEditor.statusLine",
	...
	private void calculateOutputValueOf(PTXor gate) {
		...
		LogicXor lxor = new LogicXor(inputList);
		char outputValue = lxor.getLogicResult();
