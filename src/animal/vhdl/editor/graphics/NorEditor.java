...
import animal.vhdl.logic.LogicNor;

/**
 * Editor for an nor gate
 * 
 * @author p_li
 */
public class NorEditor extends FillablePrimitiveEditor implements ItemListener,
	...
	protected void buildGUI() {
		...
		ChangeListener listener = new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				PTNor gate = (PTNor) getCurrentObject();
				if (gate != null) {
	...
	public void setProperties(XProperties props) {
		colorChooser.setColor(props.getColorProperty(PTNor.NOR_TYPE_LABEL
				+ ".color", Color.black));
		depthBox.setSelectedItem(props.getProperty(PTNor.NOR_TYPE_LABEL + ".depth",
				"16"));
		fillColorChooser.setColor(props.getColorProperty(PTNor.NOR_TYPE_LABEL
				+ ".color", Color.black));
		filledCB.setSelected(props
				.getBoolProperty(PTNor.NOR_TYPE_LABEL + ".filled"));
		// inputAmountSpinner.setValue(props.getIntProperty(PTNor.NOR_TYPE_LABEL
		// + ".inputAmount", 2));
	}

	public void getProperties(XProperties props) {
		props.put(PTNor.NOR_TYPE_LABEL + ".color", colorChooser.getColor());
		props.put(PTNor.NOR_TYPE_LABEL + ".depth", depthBox.getSelectedItem());
		props.put(PTNor.NOR_TYPE_LABEL + ".fillColor", fillColorChooser.getColor());
		props.put(PTNor.NOR_TYPE_LABEL + ".filled", filledCB.isSelected());
		// props.put(PTNor.NOR_TYPE_LABEL + ".inputAmount", inputAmountSpinner
		// .getValue().toString());
		// props.put(PTNor.NOR_TYPE_LABEL + ".outputName", String.valueOf(outText
		// .getText()));
	}
	...
	public Editor getSecondaryEditor(EditableObject go) {
		...
		NorEditor result = new NorEditor();
		result.extractAttributesFrom(go);
		return result;
	}

	public String getStatusLineMsg() {
		return AnimalTranslator.translateMessage("NorEditor.statusLine",
	...
	private void calculateOutputValueOf(PTNor gate) {
		...
		LogicNor lnor = new LogicNor(inputList);
		char outputValue = lnor.getLogicResult();
