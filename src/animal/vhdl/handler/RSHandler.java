...
/**
 * This class provides the operations that can be performed on RS.
 */
public class RSHandler extends GraphicObjectHandler {
  public Vector<String> getMethods(PTGraphicObject ptgo, Object obj) {
    Vector<String> result = new Vector<String>();

    // only works if the passed object is a PTRS!
    if (!(ptgo instanceof PTRS))
      return result;
  ...
  public void propertyChange(PTGraphicObject ptgo, PropertyChangeEvent e) {
    ...
    if ("translate".equalsIgnoreCase(what)) {
      ...
      currentSize.setSize(currentSize.getWidth() + diff.x, currentSize
          .getHeight()
          + diff.y);
