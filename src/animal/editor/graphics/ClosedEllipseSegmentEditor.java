 ...
 */
public class ClosedEllipseSegmentEditor
extends OrientedFillablePrimitiveEditor {
  ...
  public ClosedEllipseSegmentEditor() {
    super();
  }
  ...
  public boolean nextPoint(int num, Point p) {
    PTClosedEllipseSegment shape = (PTClosedEllipseSegment) getCurrentObject();
    switch (num) {
    case 1:
  ...
  public int getMinDist(PTGraphicObject go, Point p) {
    PTClosedEllipseSegment pg = (PTClosedEllipseSegment) go;
  ...
  public EditPoint[] getEditPoints(PTGraphicObject go) {
    ...
    int nr = 0;
    // result[nr++] = new EditPoint(1, helper);
  ...
  public EditableObject createObject() {
    PTClosedEllipseSegment pg = new PTClosedEllipseSegment();
    storeAttributesInto(pg);
    return pg;
  }

  public Editor getSecondaryEditor(EditableObject go) {
    ClosedEllipseSegmentEditor result = new ClosedEllipseSegmentEditor();
    // important! result must be of type CircleEditor (or cast)
    // and the parameter passed must be of type PTClosedEllipseSegment.
