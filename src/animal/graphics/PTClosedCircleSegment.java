 ...
 */
public class PTClosedCircleSegment extends ClosedArcBasedShape 
...
//	private static final long		serialVersionUID	= 4711724437159551057L;
	...
	public void paint(Graphics g) {
		paint(g, radius, radius, startAngle, totalAngle);
	...
	public int getAngle(Point p) {
		return getAngle(p, getRadius(), getRadius());
	}
...
//	  System.err.println("cw? " +isClockwise +", sa: " +getStartAngle() 
//	      +", total: " + getTotalAngle() + ", alpha: " +alpha +" => "
//	      + ((getStartAngle() + 360 + (isClockwise ? -alpha : alpha)))
//	      + " ==> " +(getStartAngle() + 360 + alpha) % 360
//        + " ===> " +(360 + alpha) % 360);
    return getPointAtLength(360 + alpha % 360, radius, radius);
//  used to be:		
    // return getPointAtLength(getStartAngle() + 360
//       (isClockwise ? -alpha : alpha)) %  360, 
//				radius, radius);
	}
	...
	public String[] handledKeywords() {
		return new String[] { "ClosedCircleSegment" };
	}
	...
	public void initializeWithDefaults(String primitiveName) {
		...
		startAngle = config.getDefaultIntValue(getType(), "startAngle", 0);
		totalAngle = config.getDefaultIntValue(getType(), "totalAngle", 90);
	}
	...
	protected void cloneCommonFeaturesInto(PTClosedCircleSegment targetShape) {
		...
		targetShape.setTotalAngle(totalAngle);
	}
	...
	public String toString() {
		...
		String prefix = toString(getType(), false, radius, radius);
		sb.append(prefix);
