 ...
 */
public class JhaveActionSupport extends BasicParser implements
	...
	public JhaveActionSupport() {
		...
		/*
		 * // System.err.println("generating ASParser"); // Parser to use parser =
		 * new AnimalscriptParser(); // Backend to use //
		 * System.err.println("generating PointMachine"); backend = new
		 * PointMachine(); try { // System.err.println("generating IModule");
		 * interactionHandler = new InteractionController(backend, false); //
		 * System.err.println("IModule generated."); } catch (BadSyntaxException e)
		 * { System.err.println("Problem: bad syntax for backend..."); }
		 * catch(Exception ee) { System.err.println("Some other exception: "
		 * +ee.getClass().getName() + " / " +ee.getMessage()); ee.printStackTrace();
		 * }
		 */
	}
	...
	public boolean generateNewStep(String command) {
		...
		return !(sameStep || employsQuestions || command
				.equalsIgnoreCase("StartQuestions"));
	}
	 ...
	 *         be at least 0
	 */
	private int getPointsPossibleForQuestion(StreamTokenizer strTok,
			String message) {
...
//			int pointsPossible = 
	...
	public void parseStartQuestions() throws IOException {
		ParseSupport.parseMandatoryWord(stok, "'StartQuestions' keyword",
				"StartQuestions");
		// System.err.println(
		// "Start Questions encountered -- now have to stop here, pass to Chris");
		StringBuilder sb = new StringBuilder(32768);
		// parse all contents without looking at them!
		// retrieve the buffered reader...
		// System.err.println("underlying reader: " +underlyingReader);
		// if (underlyingReader != null)
		// System.err.println(underlyingReader.getClass().getName());
		BufferedReader bReader;
		if (underlyingReader instanceof BufferedReader)
			bReader = (BufferedReader) underlyingReader;
		else
			bReader = new BufferedReader(underlyingReader);
		String cLine = null;
		while ((cLine = bReader.readLine()) != null)
			sb.append(cLine).append("\n");
		// System.err.println(">>>>>>" +sb.toString() +"<<<<<");
		// now ask Chris to parse the rest of the script...
		startQuestionsEncountered = true;
		employsQuestions = true;
		ExternalAction.resetActions();
		// QuestionFactory.parseScript(sb.toString());
	}
	...
	public void parseInteractionDefinition() throws IOException {
		// System.err.println("Parsing 'interactionDefinition' command");
		...
		// System.err.println("INTDEF: " +interactionURL);

		try {
			/*
			 * System.out.println("Now starting to generate the module...");
			 * interactionHandler = new InteractionController(backend, true);
			 * System.out.println("done starting to generate the module...");
			 */
			System.err.println("assigning intDef file " + interactionURL);
			interactionHandler.interactionDefinition(interactionURL,
					"text/animalscript");
		}
		...
		// }
			...
			System.err.println("Some other exception: " + ee + "// " + ee.getMessage());
		}

		// System.err.println("Created handler: " + interactionHandler);
		// System.err.println("setting definition URL to " + interactionURL);
		...
		InteractionElement interactionElement = new InteractionElement(currentStep,
				...
				interactionHandler);
		// System.err.println("iHandler: " + interactionHandler);
