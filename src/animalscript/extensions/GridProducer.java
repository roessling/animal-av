 ...
 * 
 */
public class GridProducer extends BasicParser {
...
//        brackets[part].setFwArrow(false);
    ...
    for (int line = 0; line < lines; line++)
      for (int col = 0; col < cols; col++) {
        ...
        boxSize[1] = backgroundRects[line][col].getNodeAsPoint(2).y
  ...
  public static void swapGridValues(String gridName1, int line1, int col1,
    ...
    System.err.println("GP.sGV(" + gridName1 + ", " + line1 +", " +col1 +" / " +gridName2
        +", " +line2 +", " +col2 +" - step " + step);
  ...
  public static void refreshGrid(String gridName, int step, int duration,
    ...
    for (int l = 0; l < size[0]; l++)
      for (int c = 0; c < size[1]; c++) {
    	  // Update old_PTGraphicObject
        PTPolygon polygonToUpdate = ((PTPolygon)animState.getCloneByNum(getObjectIDs().getIntProperty(gridBackKey(gridName, l, c))));
        if(polygonToUpdate!=null){
	    	PTPoint pa, pb, pc, pd;
	        pa = new PTPoint(backPositions[l][c][0], backPositions[l][c][1]);	// ObenLinks
	        pb = new PTPoint(backPositions[l][c][0], backPositions[l][c][1]
	              + backPositions[l][c][3]);									// UntenLinks
	        pc = new PTPoint(backPositions[l][c][0] + backPositions[l][c][2],
	        		  backPositions[l][c][1] + backPositions[l][c][3]);				// UntenRechts
	        pd = new PTPoint(backPositions[l][c][0] + backPositions[l][c][2],	// ObenRechts
	        		  backPositions[l][c][1]);
            polygonToUpdate.getNodes().get(0).set(pa.getLocation());
            polygonToUpdate.getNodes().get(1).set(pb.getLocation());
            polygonToUpdate.getNodes().get(2).set(pc.getLocation());
            polygonToUpdate.getNodes().get(3).set(pd.getLocation());
        }
        
        PTText textToUpdate = ((PTText)animState.getCloneByNum(getObjectIDs().getIntProperty(gridTextKey(gridName, l, c))));
        if(textToUpdate!=null){
      	  textToUpdate.setText(getObjectProperties().getProperty(gridTextStringKey(gridName, l, c)));
      	  textToUpdate.setLocation(getObjectProperties().getPointProperty(gridTextPositionKey(gridName, l, c)));
        }
    ...
    while (collectedResizeKeys.hasMoreElements()) {
      ...
      PTPolyline currentMoveBaseX = new PTPolyline(new int[] { 0,
          ...
          new int[] { 0, 0 }); // array of ycoords
      currentMoveBaseX.setDepth(10);
      currentMoveBaseX.getNum(true);
      currentMoveBaseX.setColor(new Color(0.5f, 0.75f, 0.25f));

      PTPolyline currentMoveBaseY = new PTPolyline(new int[] { 0, 0 }, // array
                                                                        // of
                                                                        // xcoords
          new int[] { 0, 0 + Integer.parseInt(currentMovementArray[1]) }); // array
                                                                            // of
                                                                            // ycoords
